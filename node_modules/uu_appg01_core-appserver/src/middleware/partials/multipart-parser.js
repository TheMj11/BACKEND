...
function multipartParser(req, res, next) {
  ...
  let done = err => {
    ...
    // some errors may occur after server stops processing the file
    // this listener is removed later by context-store
    busboy.on("error", err => {
      logger.debug("Parsing multipart form ended with error.", err);
    });
