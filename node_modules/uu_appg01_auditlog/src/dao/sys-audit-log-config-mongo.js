...
class SysAuditLogConfigMongo extends UuObjectDao {
  ...
  async obtainPruneLock(awid, lockedBy = "", pruneLockTtl = 0) {
    ...
    let object = await collection.findOneAndUpdate(filter, update, { includeResultMetadata: true });
    return object && object.lastErrorObject && object.lastErrorObject.updatedExisting;
  }

  async obtainMetricsExportLock(awid, lockedBy, pruneLockTtl = 0) {
    let date = new Date();
    let filter = {
      awid,
      $or: [
        {
          "metricsExportLock.lockedSince": {
            $exists: false
          }
        },
        {
