...
const uuAppMetricsEnabled = Config.getBoolean("uu_app_metrics_enabled");
...
class RequestAuditLogAbl{
  ...
  constructor() {
    if (uuAppMetricsEnabled) {
      ...
      const uuAwsUseCaseMeter = meterProvider.getMeter("uuAwsUseCase");
      const uuAsidUseCaseMeter = meterProvider.getMeter("uuAsidUseCase");

      this._awsDtoInSizeAccumulator = new SyncingMetricAccumulator(uuAwsUseCaseMeter, "DtoInSizeSum");
      this._awsDtoOutSizeAccumulator = new SyncingMetricAccumulator(uuAwsUseCaseMeter, "DtoOutSizeSum");
      this._awsCallDurationAccumulator = new SyncingMetricAccumulator(uuAwsUseCaseMeter, "CallDurationSum");
      this._asidDtoInSizeAccumulator = new SyncingMetricAccumulator(uuAsidUseCaseMeter, "DtoInSizeSum");
      this._asidDtoOutSizeAccumulator = new SyncingMetricAccumulator(uuAsidUseCaseMeter, "DtoOutSizeSum");
      this._asidCallDurationAccumulator = new SyncingMetricAccumulator(uuAsidUseCaseMeter, "CallDurationSum");
    }
  }
  ...
  async logRequest(code, req, res, asyncWrite = true) {
    ...
    let { uri, session, authorizationResult, dispatchedError } = res.locals;
    ...
    let requestSize = (req.headers["content-length"] && Number(req.headers["content-length"])) || 0;
    ...
    switch(schemaVersion){
      ...
      default:
        ...
        logData.requestSize = requestSize;
  
        if(code === AUDITLOG_START_MIDDLEWARE_CODE){
          eventCode = "uuApp/useCaseExecutionStart";
        }else{
          logData.responseSize = res.responseBodySize || 0;
    ...
    if (uuAppMetricsEnabled) {
      let attributes = { awid: uri.getAwid() };
      if (code === AUDITLOG_START_MIDDLEWARE_CODE) {
        this._awsDtoInSizeAccumulator.add(requestSize, attributes);
        this._asidDtoInSizeAccumulator.add(requestSize, { asid });
      } else {
        if (!res.locals.auditLogStartHasLogged) {
          this._awsDtoInSizeAccumulator.add(requestSize, attributes);
          this._asidDtoInSizeAccumulator.add(requestSize, { asid });
        }
        this._awsDtoOutSizeAccumulator.add(res.responseBodySize || 0, attributes);
        this._awsCallDurationAccumulator.add(duration, attributes);
        this._asidDtoOutSizeAccumulator.add(res.responseBodySize || 0, { asid });
        this._asidCallDurationAccumulator.add(duration, { asid });
      }
    }
