...
try {
  ...
  console.warn(
    "Library uu_appg01_oidc is not installed, ensure other realization of AuthenticationService is available.",
    e
  );
}
...
class TestHelper {
  ...
  /**
   * Returns db connection for given connection uri.
   * @param {string} dbName Name of the database.
   * @param {boolean} dbClient If true, returns db client instead of db connection.
   * @returns {Promise<DbConnection>}
   */
  async getDbConnection(dbName = "primary", dbClient = false) {
    const dataStoreMap = Config.get("uuSubAppDataStoreMap") || {};
    if (!dataStoreMap[dbName]) {
      throw new BaseError(
        `Database ${dbName} is not found in test configuration. Please make sure it is configured in uuSubAppDataStoreMap and that you have either started the test mongoDB server via TestHelper.setup() or put in a mongodb connection uri directly in test.json.`
      );
    }
    const connectionString = dataStoreMap[dbName];
    if (dbClient) {
      return await DbConnection.getDbClient(connectionString);
    }
    return await DbConnection.get(connectionString);
  }
  ...
  async teardown() {
    ...
    await DbConnection.closeAllConnections();
    this._binaryg02Dbs = {};
  }
  ...
  async executePostCommand(useCase, dtoIn = {}, session = null, opts = {}) {
    if (typeof opts === "string") {
      // support awid for backward compatibility, TODO remove in the next major version
      console.warn(
        'Using awid as a 4th parameter of executePostCommand method is deprecated. Use object {awid: "..."} instead. This method overload is deprecated and is going to be removed!'
      );
      opts = {
        awid: opts,
      };
  ...
  async executeGetCommand(useCase, dtoIn = {}, session = null, opts = {}) {
    if (typeof opts === "string") {
      // support awid for backward compatibility, TODO remove in the next major version
      console.warn(
        'Using awid as a 4th parameter of executeGetCommand method is deprecated. Use object {awid: "..."} instead. This method overload is deprecated and is going to be removed!'
      );
      opts = {
        awid: opts,
      };
   ...
   * @returns {Promise<AuthenticationService.Session>} Instance of user session
   */
    ...
    for (let key of Object.keys(configObject)) {
      ...
      if (binaryStoreUriMatch) {
        ...
        if (
          binaryStoreUri === null ||
          binaryStoreUri === "" ||
          (binaryStoreUri.startsWith("mongodb") && !(await this._isMongoStarted(binaryStoreUri)))
        ) {
  ...
  async _dropMongoDb(connectionUri) {
    const db = await DbConnection.get(connectionUri);
    return await db.dropDatabase();
  }

  /**
   * Returns instance of the TestMongo. Starts a new one if it does not exist.
   * @return {Promise<TestMongo>} test MongoDb.
   * @private
   */
  async _getTestMongo() {
    if (!this._testMongo) {
