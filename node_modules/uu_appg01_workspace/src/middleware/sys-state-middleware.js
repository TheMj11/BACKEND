...
class SysStateMiddleware {
  ...
  async _redirectToDefaultErrorPage(res, error) {
    ...
    let defaultAdviceNote = "";
    if(params.adviceNote) {
		if (params.adviceNote.message) {
			defaultAdviceNote += `<p><b>Advice note message:</b> ${params.adviceNote.message}</p>`
		}
		if (params.adviceNote.startTime) {
			defaultAdviceNote += `<p><b>Start time:</b> ${params.adviceNote.startTime}</p>`;
		}
		if (params.adviceNote.estimatedEndTime) {
			defaultAdviceNote += `<p><b>Estimated end time:</b> ${params.adviceNote.estimatedEndTime}</p>`;
		}
	}
    
    const isAwid = params.type === "awid";
    params.estimatedEndTime = params.formattedEstimatedEndTime ? `before ${params.formattedEstimatedEndTime}.` : "soon.";
    params.infoText = `${isAwid ? "uuAppWorkspace" : "Application"} is temporary unavailable. The ${isAwid ? "Awid" : "Asid"} SysState is set to "${params.currentSysState}"`;
    params.defaultAdviceNote = defaultAdviceNote; 
    await ErrorPageHelper.respondWithErrorPage({ res, params: params, viewFile: "sys-state-error-page.html", error });
  }
