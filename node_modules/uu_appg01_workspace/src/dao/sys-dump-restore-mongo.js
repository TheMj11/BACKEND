...
class SysDumpRestoreMongo extends UuObjectDao {
  ...
  async createIndexes(indexSpecs, isAsid, schema, uuAppErrorMap) {
    let db = await DbConnection.get(this.customUri);
    for (const index of indexSpecs) {
      delete index["ns"];
      ...
      try {
        ...
        if (isAsid) {
          logger.warn(`Create index (coll. name: ${this.collectionName}, schema: ${schema}) by DAO failed. An drop and create attempt will be made.`, e);
          try {
            await db.collection(this.collectionName).dropIndex(index.name);
            await db.collection(this.collectionName).createIndexes([index]);
          } catch (e1) {
            this._addFailedIndexCreationWarning(uuAppErrorMap, schema, index, e1);
          }
        } else {
          this._addFailedIndexCreationWarning(uuAppErrorMap, schema, index, e);
        }
      }
    }
  }
  ...
  _addFailedIndexCreationWarning(uuAppErrorMap, schema, index, error) {
    let invalidRow = { schema, index, cause: error };
