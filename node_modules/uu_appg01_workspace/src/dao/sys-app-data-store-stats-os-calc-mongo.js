const { DbConnection } = require("uu_appg01_datastore");
const { LoggerFactory } = require("uu_appg01_core-logging");

class SysAppDataStoreStatsOsCalcMongo {

  constructor(connectionString) {
    this.logger = LoggerFactory.get("uuapp.workspace.SysAppDataStoreStatsOsCalcMongo");
    connectionString += connectionString.includes("?") ? "&" : "?";
    connectionString += "readPreference=secondaryPreferred";
    this.connectionString = connectionString;
    DbConnection.init(connectionString);
  }

  async calculate(awid, collectionName) {
    let db = await DbConnection.get(this.connectionString);
    let agggregateCalcSupported = await this._isAggregateCalculationSupported(db);

    let objectStats;

    if (agggregateCalcSupported) {
      this.logger.debug(`Calculating statistics via aggregation (${awid}, ${collectionName})`);

      let aggregation;
      if (collectionName === "sysUuSubAppInstance" || collectionName === "sysUuAppMetrics") {
        aggregation = [
          { $match: { asid: awid } },
          { $group: { _id: { asid: "$asid" }, count: { $sum: 1 }, dataSize: { $sum: { $bsonSize: "$$ROOT" } } } },
          { $project: { _id: 0, dataSize: 1, count: 1 } }
        ];
      } else {
        aggregation = [
          {$match: { awid: awid }},
          { $group: { _id: { awid: "$awid" }, count: { $sum: 1 }, dataSize: { $sum: { $bsonSize: "$$ROOT" } } } },
          { $project: { _id: 0, count: 1, dataSize: 1 } }
        ];
      }

      objectStats = await db.collection(collectionName).aggregate(aggregation).next();
      if (!objectStats) {
        objectStats = { count: 0, dataSize: 0 };
      }

    } else {
      this.logger.debug(`Calculating statistics via map reduce (${awid}, ${collectionName})`);

      let query;
      if (collectionName === "sysUuSubAppInstance" || collectionName === "sysUuAppMetrics") {
        query = { asid: awid };
      } else {
        query = { awid }
      }

      const mapReduceCmd = {
        mapReduce: collectionName,
        map: "function () { emit(0, {count: 1, dataSize: Object.bsonsize(this)}); }",
        reduce: "function (k, infos) {" +
                  "var count = 0;" +
                  "var dataSize = 0;" +
                  "infos.forEach(function (info) {" +
                    "count += info.count;" +
                    "dataSize += info.dataSize;" +
                  "});" +
                  "return {count: count, dataSize: dataSize};" +
                "}",
        out: { inline: 1 },
        query
      };

      let collectionInfo = await db.command(mapReduceCmd);

      if (!collectionInfo.results[0]) {
        objectStats = { count: 0, dataSize: 0 };
      } else {
        objectStats = collectionInfo.results[0].value;
      }

    }

    if (objectStats["count"] > 0) {
      let collStats;
      collStats = await db.command({ "collStats": collectionName });

      let totalIndexSize = collStats["totalIndexSize"];
      let totalCount = collStats["count"];
      objectStats.indexSize = ((totalIndexSize / totalCount) * objectStats["count"]);
      objectStats.size = objectStats.dataSize + objectStats.indexSize;

    } else {
      objectStats.size = objectStats.indexSize = 0;
    }

    return objectStats;
  }

  async getStats() {
    let db = await DbConnection.get(this.connectionString);
    return db.stats();
  }

  async getCollectionStats(collectionName) {
    let db = await DbConnection.get(this.connectionString);
    let res = await db
      .collection(collectionName)
      .aggregate([{ $collStats: { storageStats: {} }}])
      .toArray();
    let storageStats = res[0].storageStats;
    return {
      size: storageStats.size,
      count: storageStats.count,
      indexSize: storageStats.totalIndexSize,
    };
  }

  async _isAggregateCalculationSupported(db) {
    try {
      let buildInfo = await db.command({ "buildInfo": 1 });
      let majorVersion = buildInfo.version.split(".")[0];
      return Number(majorVersion) >= 5;
    } catch (e) {
      this.logger.warn("Failed to evaluate DB version for stat calculation.", e);
      return false;
    }
  }
  
  async getCollectionNames() {
    let db = await DbConnection.get(this.connectionString);
    return (await db.collections()).map(c => c.collectionName);
  }

}

module.exports = SysAppDataStoreStatsOsCalcMongo;
