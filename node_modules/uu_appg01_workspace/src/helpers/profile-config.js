...
const DEFAULT_TIER_LIST = ["standard"]; // TODO
const DEFAULT_LICENSE_POLICY = "licensed";
...
class ProfileConfig extends BaseConfig {
  ...
  loadProfilesConfig(ignoreAsidLicenseOwnerListValidation = false) {
    ...
    this._configureDefaultValues(profilesConfig);
  ...
  /**
   * Configures default values for both asid and awid profile config.
   * @param profilesConfig
   * @private
   */
  _configureDefaultValues(profilesConfig) {
    // privileged profiles
    profilesConfig[PROFILE_CONFIG_PRIVILEGED_LABEL].tierList ||= DEFAULT_TIER_LIST;
    ...
    Object.keys(privilegedUseCaseMap).forEach(useCaseName => {
      ...
      useCase.disabledInTierList ||= [];
      useCase.licensePolicy ||= DEFAULT_LICENSE_POLICY;
    });

    profilesConfig["*"].tierList ||= DEFAULT_TIER_LIST;
