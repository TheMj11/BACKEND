...
const LogStoreService = require('../helpers/log-store-service.js');
const LogStoreHelper = require('../helpers/log-store-helper.js');
...
class SysAppInstanceAbl {
  ...
  async setLicense(asid, dtoIn) {
    // HDS 1. - Validation of dtoIn.
    let validationResult = this.validator.validate("sysUuSubAppInstanceSetLicenseDtoInType", dtoIn);
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    // HDS 2. - System updates licence infomation to sysUuSubAppInstance (sysUuSubAppInstance DAO update).
    let dtoOut;
    try {
      dtoOut = await this.dao.update({ asid, license: dtoIn });
    } catch (e) {
      if (e.cause instanceof ObjectNotFound) {
        // A3
        throw new Errors.SetLicense.SysUuSubAppInstanceDoesNotExist({ uuAppErrorMap }, { asid });
      }
      throw e;
    }
  ...
  async getLogAccessAttributes(dtoIn) {
    // HDS 1
    let validationResult = this.validator.validate("sysUuSubAppInstanceGetLogAccessAttributesDtoInType", dtoIn);
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    // HDS 3
    let link = LogStoreHelper.createGetRecordListLink(accessKeyAttrs);
  ...
  async getLicense(dtoIn) {
    let uuAppErrorMap = {};

    let validationResult = this.validator.validate("sysUuSubAppInstanceGetLicenseDtoInType", dtoIn);
    ...
    let license;
    try {
      license = await WorkspaceLicenseService.getLicense(this.asid);
    } catch (e) {
