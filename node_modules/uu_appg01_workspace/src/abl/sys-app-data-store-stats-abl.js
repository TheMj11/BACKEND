...
class SysAppDataStoreStatsAbl {
  constructor() {
    ...
    this.calcLockDurationSec = Config.getNumber("uu_app_server_data_store_stats_calculation_lock_ttl");
    const calcPeriod = Config.getNumber("uu_app_server_data_store_stats_calculation_period");
    if (this.calcLockDurationSec && calcPeriod && this.calcLockDurationSec > calcPeriod) {
      this.calcLockDurationSec = calcPeriod;
    }
  }
  ...
  async calculate(awid, dtoIn, runSync = false) {
    ...
    if (this.calcLockDurationSec) {
      limits.calcLockDurationSec = this.calcLockDurationSec;
    }
  ...
  removeUnusedStats(stats) {
    Object.keys(stats.dataStoreMap).forEach(dataStoreName => {
      ...
      if (dataStore.type !== "uuAppBinaryStoreg02") {
  ...
  async _calculateDataStoreStatsAsync(stats, lock, uuAppErrorMap) {
    ...
    let promises = Array.from(Object.keys(stores)).map(async dataStoreName => {
      ...
      let isUuAppBinaryStoreg02 = false;
      let calcDaoClass;
      ...
      } else if (dataStore.type[0] === "uuAppBinaryStoreg02" && dataStore.type[1] === "service") {
        isUuAppBinaryStoreg02 = true;
      } else {
        //A7
      ...
      //HDS II.4.2.1
      try {
        calcDao = !isUuAppBinaryStoreg02 ? new calcDaoClass(dataStore.connectionString) : null;
      } catch (e) {
        //throw error
      }
      ...
      if (!dataStoreStats) {
        dataStoreStats = isUuAppBinaryStoreg02 ? {
      ...
      if (stats["awid"] === this.asid && !isUuAppBinaryStoreg02) {
        const stats = await calcDao.getStats();
        dataStoreStats.dsDataSize = stats.dataSize;
        dataStoreStats.dsSysSize = 0;
        dataStoreStats.dsStorageSize = stats.storageSize;
        dataStoreStats.dsIndexSize = stats.indexSize;
        dataStoreStats.dsSysIndexSize = 0;
        dataStoreStats.dsDataCount = stats.objects;
        dataStoreStats.dsSysCount = 0;
      }


      //HDS II.4.2
      let promises = [];
      if (!isUuAppBinaryStoreg02) {
        let persistentSchemes = await this._getPersistentSchemes(calcDao, uuAppErrorMap);
        promises = Array.from(dataStore.schemaList).map(async schema => {
          let schemaStats;
          
          if (persistentSchemes.includes(schema)) {
            try {
              if (stats["awid"] === this.asid && schema.startsWith("sys")) {
                const collectionStats = await calcDao.getCollectionStats(schema);
                dataStoreStats.dsSysSize += collectionStats.size;
                dataStoreStats.dsSysIndexSize += collectionStats.indexSize;
                dataStoreStats.dsSysCount += collectionStats.count;
              }
            ...
            logger.warn(`Statistics calculation for the "${schema}" scheme will be skipped because there is no DB collection for it.`);
            schemaStats = {count: 0, dataSize: 0, indexSize: 0, size: 0};
          }
      ...
      //HDS II.4.3
      if (isUuAppBinaryStoreg02) {
        const getStatisticsUri = UriBuilder.parse(dataStore.connectionString).setUseCase("appBinaryStore/getStatistics");
        const sysIdentitySession = await AuthenticationService.authenticate({ systemIdentity: this.asid });

        const uuAppBinaryStoreg02StatsPromise = new Promise(async (resolve, reject) => {
          await lock.setExpiration(limits.calcLockDurationSec);

          let now = new Date();
          let getStatisticsDtoOut;

          try {
            getStatisticsDtoOut = await AppClient.cmdGet(getStatisticsUri, { clientAwid: stats["awid"] }, { session: sysIdentitySession });
          } catch (e) {
            reject(e);
            return;
          }

          stats["dataStoreMap"][dataStore.name]["size"] = getStatisticsDtoOut.totalBinariesSize;
          stats["dataStoreMap"][dataStore.name]["count"] = getStatisticsDtoOut.numberOfBinaries;
          stats["dataStoreMap"][dataStore.name]["ts"] = now;
        ...
        promises.push(uuAppBinaryStoreg02StatsPromise);
      }
  ...
  _recalculateTotals(stats) {
    ...
    stats["sysSize"] = 0;
    stats["indexSize"] = 0;
    stats["sysIndexSize"] = 0;
    Object.keys(stats.dataStoreMap).forEach(dataStoreName => {
      let dataStore = stats.dataStoreMap[dataStoreName];
      if (dataStore.type !== "uuAppBinaryStoreg02") {
        dataStore["count"] = 0;
        dataStore["sysCount"] = 0;
        dataStore["size"] = 0;
        dataStore["dataSize"] = 0;
        dataStore["indexSize"] = 0;
        dataStore["sysIndexSize"] = 0;
        dataStore["sysSize"] = 0;
        Object.keys(dataStore.schemaMap).forEach(schemaName => {
          let schema = dataStore.schemaMap[schemaName];
          ...
          if (schemaName.startsWith("sys")) {
            dataStore["sysIndexSize"] += schema["indexSize"];
            dataStore["sysCount"] += schema["count"];
            dataStore["sysSize"] += schema["dataSize"];
          }
        });
      }

      stats["count"] += dataStore["count"];
      stats["sysCount"] += dataStore["sysCount"] || 0;
      stats["size"] += dataStore["size"];
      // In uuAppBinaryStoreg02 stats "size" and "dataSize" represent the same value.
      stats["dataSize"] += dataStore.type === "uuAppBinaryStoreg02"
        ? dataStore["size"]
        : dataStore["dataSize"];
      stats["indexSize"] += dataStore["indexSize"] || 0;
      stats["sysIndexSize"] += dataStore["sysIndexSize"] || 0;
      stats["sysSize"] += dataStore["sysSize"] || 0;
    });
  }
  ...
  _loadDataStoresMap() {
    ...
    // Load uuAppBinaryStoreg02 configuration
    const config = Config.toJSON();
    Object.keys(config).forEach(key => {
      const match = key.match(/^uu_app_binarystore(?:_(.+))?_uri$/);
      if (!match) {
        return;
      }

      const uuAppBinarystoreUri = config[match[0]];
      if (uuAppBinarystoreUri.startsWith("mongodb://")) {
        logger.warn("UuAppBinaryStoreg02 MongoDB realization is not supported, statistics wont be calculated.");
        return;
      }

      let storeName = match[1] ? match[1] : "uuAppBinaryStoreg02";
      if (realizationStructure[storeName]) {
        storeName = `uuAppBinaryStoreg02_${match[1]}`;
      }

      realizationStructure[storeName] = {
        name: storeName,
        type: ["uuAppBinaryStoreg02", "service"],
        connectionString: uuAppBinarystoreUri
      };
    })
  ...
  async _getPersistentSchemes(calcDao, uuAppErrorMap) {
    try {
      return await calcDao.getCollectionNames();
    } catch(e) {
      logger.error(`Failed to obtain scheme names from storage.`, e);
      throw new Errors.StatsCalculate.GetSchemeNamesFailed({ uuAppErrorMap }, { cause: e });      
    }
  }
}
