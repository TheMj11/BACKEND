...
class SysDumpRestoreAbl {
    ...
    if (bulkQueueSize.value > RESTORE_QUEUE_LIMIT || force) {
      ...
      try {
        ...
        if (isStrictMode) {
          ...
          let invalidRows = [];
          let bulkWriteResult = e.result;
          if (bulkWriteResult) {
            let insertedIds = bulkQueue.slice(bulkWriteResult.insertedCount);
            insertedIds.forEach(insertedId => {
              invalidRows.push({ schema, objectId: insertedId.data._id, cause: e.message });
            });
          }
  ...
  async _saveSchema(filter, dao, archive, awidList) {
    ...
    pump(documents.stream(), stringifyStream);
