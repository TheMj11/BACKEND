...
const PARAM_AUTHORIZATION_RETRY_ATTEMPTS = "uu_app_workspace_authorization_retry_attempts";
const DEFAULT_AUTHORIZATION_TIMEOUT = 5 * 1000;
const DEFAULT_AUTHORIZATION_RETRY_ATTEMPTS = 0;

const PARAM_ROLE_CAST_CACHE_TTL = "uu_app_workspace_role_cast_cache_ttl";

const PARAM_ARTIFACT_AUTHORIZATION_STRATEGY = "uu_app_workspace_artifact_authorization_strategy";
const ARTIFACT_AUTHORIZATION_STRATEGY_MAP = {
  workspace: "workspace",
  usecase: "usecase",
};
const DEFAULT_ARTIFACT_AUTHORIZATION_STRATEGY = ARTIFACT_AUTHORIZATION_STRATEGY_MAP.workspace;
...
class WorkspaceAuthorizationService {
  ...
  constructor(options = {}) {
    ...
    this._artifactAuthorizationStrategy = opts.getString(
      PARAM_ARTIFACT_AUTHORIZATION_STRATEGY,
      DEFAULT_ARTIFACT_AUTHORIZATION_STRATEGY
    );
    ...
    this._authRetryAttempts = opts.getNumber(PARAM_AUTHORIZATION_RETRY_ATTEMPTS, DEFAULT_AUTHORIZATION_RETRY_ATTEMPTS);
  ...
  async authorize(session, uri, authorizationData = {}) {
    ...
    if (this._artifactAuthorizationStrategy === ARTIFACT_AUTHORIZATION_STRATEGY_MAP.usecase) {
      authorizationData.useCase = useCaseName;
    }
    ...
    if (useCaseProfileList && useCaseProfileList.length) {
      let mergedUcProfiles = ucProfiles.concat(useCaseProfileList);
      ucProfiles = [...new Set(mergedUcProfiles)];
    }
    ...
    if (authorizedProfiles.length === 0 && !ucProfiles.includes(PUBLIC_PROFILE)) {
      ...
      if (Date.now() - authorizationTime > this._userProfileNegativeCacheTtl) {
        ...
        if (result.useCaseProfileList && result.useCaseProfileList.length) {
          let mergedUcProfiles = ucProfiles.concat(result.useCaseProfileList);
          ucProfiles = [...new Set(mergedUcProfiles)];
        }
      } else {
  ...
  async _getUserProfiles(awid, uuIdentity, session, ucProfiles, authorizationData = {}, useCache = true) {
    let cacheKey;
    let { useCase = null } = authorizationData;
    if (useCase) {
      cacheKey = `${awid}|${uuIdentity}|${useCase}`;
    } else {
      cacheKey = `${awid}|${uuIdentity}`;
    }
    ...
    if (!useCache || !result) {
      ...
      if (useCase) {
        ...
        result.useCaseProfileList = authorizationResult.ucAppProfiles;
      }
  ...
  async _loadUserProfiles(awid, uuIdentity, session, ucProfiles, authorizationData = {}) {
    ...
    switch (authorizationStrategy) {
      ...
      case ARTIFACT_AUTHZ_STRATEGY:
        try {
          ...
          result = await this._loadUserProfilesByArtifact(session, uuIdentity, authzArtifactUri, uuAppMetaModelVersion, useCase);
        } catch (e) {
    ...
    if (result) {
      userProfiles.push(...result.userProfiles);
      resultComplete = resultComplete && result.resultComplete;
      if (result.ucAppProfiles && result.ucAppProfiles.length) {
        ucAppProfiles.push(...result.ucAppProfiles);
      }
      if (!error) {
        error = result.error;
      }
    ...
    return { userProfiles, ucAppProfiles, resultComplete, doCache, authorizationStrategy, error };
  }
  ...
  async _loadUserProfilesByArtifact(session, uuIdentity, authzArtifactUri, uuAppMetaModelVersion, useCase, attempt = 0) {
    let userProfiles = [];
    ...
    try {
      ...
      if (result.ucUuAppProfileList) {
        ucAppProfiles.push(...result.ucUuAppProfileList);
      }
      if (result.ucUuAppRoleProfileList) {
        ucAppProfiles.push(...result.ucUuAppRoleProfileList);
      }
    } catch (e) {
      if (
        e.status === 403 ||
      ...
      } else if (this._authRetryAttempts > 0 && attempt < this._authRetryAttempts) {
        logger.debug(
          `Loading of profiles for user ${uuIdentity} failed (authorization uri: ${uriBuilder}). Retry conditions valid, retrying the request.`,
          e,
        );
        return await this._loadUserProfilesByArtifact(
          session,
          uuIdentity,
          authzArtifactUri,
    ...
    return { userProfiles, userAppTypeProfiles, authorizedAppTypeProfiles, ucAppTypeProfiles, ucAppProfiles, resultComplete, error };
  }
