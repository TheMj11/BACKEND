...
class SysAppWorkspaceController {
  ...
  setLicense(ucEnv) {
    const dtoIn = ucEnv.getDtoIn();
    return SysAppWorkspaceAbl.setLicense(dtoIn);
  }
  ...
  getLogAccessAttributes(ucEnv) {
    const uri = ucEnv.uri;
    const dtoIn = ucEnv.getDtoIn();
    let result = SysAppWorkspaceAbl.getLogAccessAttributes(uri, dtoIn);
    return result;
  }
  ...
  getLicense(ucEnv) {
    ...
    const authorizedProfiles = ucEnv.getAuthorizationResult().getAuthorizedProfiles();
    return SysAppWorkspaceAbl.getLicense(awid, dtoIn, uuIdentity, authorizedProfiles);
  }
}
