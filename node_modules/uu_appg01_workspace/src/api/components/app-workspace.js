...
const { Config, Finder, OptsReader } = require("uu_appg01_core-utils");
...
class AppWorkspace {
  constructor() {
    ...
    this._licensingEnabled = opts.getBoolean("uu_app_workspace_awid_license_verification_enabled", false);
    this._uuAppMetaModelVersion = AppWorkspaceAbl.uuAppMetaModelVersion;
  ...
  /**
   * Sets awid to "beingInitialized" sysState
   *
   * @param {string} awid awid
   * @param {Object} [adviceNote] note about current awid sysState
   * @returns {Promise<Object>} uuObject sysAppWorkspace
   */
  async setBeingInitializedSysState(awid, adviceNote = {}) {
    return await AppWorkspaceAbl.setBeingInitializedSysState(awid, { adviceNote });
  }
 ...
 */
  async getSysData(uri, session) {
    return await AppWorkspaceAbl.getSysData(uri, session);
  }
  ...
  /**
   * Checks if Awid license verification is enabled.
   * @returns {Boolean} true if enabled
   */
  isLicenseVerificationEnabled() {
    return this._licensingEnabled;
  }

  getUuAppMetaModelVersion() {
    return this._uuAppMetaModelVersion;
  }
}
