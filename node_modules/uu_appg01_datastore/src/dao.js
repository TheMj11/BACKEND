...
const logger = LoggerFactory.get("uu.app.objectstore.UuDataStoreDao");
...
class UuDataStoreDao {
  constructor(name, maxNoi, maxSoi, customUri, connectionOpts = {}) {
    ...
    this._indexPromise = this._createDefaultIndexes();
  }
      ...
      return await db
        ...
        .then((result) => {
          if (!result) {
            ...
            let lockedObject = self._convertToId(result);
  ...
  async unlockOne(filter, lock) {
    ...
    if (!(lock instanceof Lock)) {
      ...
      try {
        ...
        return await db
          .collection(this.collectionName)
          .findOneAndUpdate(filter, unlockEdit, { returnDocument: "after", includeResultMetadata: true })
          .then((result) => {
  ...
  async closeDB() {
    ...
    try {
      result = mutex.runOneAtTime(this.customUri, async () => { await dbClient.close() });
    } catch (e) {
      logger.warn(`Failed to close DB connection for URI: ${this.customUri}`, e);
    }
    return result;
  }
