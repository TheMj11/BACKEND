...
class OidcController {
  ...
  constructor() {
    ...
    this._callbackUve = Fs.readFileSync(Path.join(__dirname, "callback.html"), "utf8");
    // autobind
   ...
   * also obtains token and stores it to cookie.
   */
  async callback(ucEnv) {
    ...
    if (authorizationCode) {
      ...
      let baseUriAlias = ucEnv.getBaseUriAlias();
      let dtoIn = ucEnv.getDtoIn();
      try {
        let scopePart = uri.getBaseUri().toString();
        let scope = [CLIENT_ACR_SCOPE, scopePart];
        let redirectUriCheck = this._getRedirectUri(uri, baseUriAlias).split("?")[0];
        ...
        this._logger.error("Unable to get token via authorization code.", e);
        ...
        let redirectUri = this._getRedirectUri(uri, baseUriAlias);
    ...
    response.setBody(this._callbackUve);
    return null;
  }

  /**
   * Refreshes token stored in session cookie
  ...
  /**
   * Refreshes token stored in session cookie (via backend channel; to be used
  ...
  async extendSession(ucEnv) {
    ...
    try {
      ...
      session._idToken = token.id_token;
      session._accessToken = token.access_token;
      token = await session.getCallToken(null, {useTokenExchange: false, excludeAuthenticationType: true});
    } catch (e) {
      if (e instanceof InvalidCredentials) {
  ...
  async logout(ucEnv) {
    ...
    if (endSessionUri) {
      ...
      if (!this._isRedirectUriValid(redirectUri, uri, ucEnv.getBaseUriAlias())) {
        redirectUri = null;
      }
    }
    if (redirectUri) {
   ...
   * @private
   */
  _getRedirectUri(uri, baseUriAlias = null) {
    ...
    if (!callbackUri || !this._isRedirectUriValid(callbackUri, uri, baseUriAlias)) {
  ...
  /**
   * Checks if redirectUri is valid in scope of uuApp.
   * @private
   */
  _isRedirectUriValid(redirectUri, requestUri, baseUriAlias = null) {
    if (redirectUri.includes("://")) {
      // In case of absolute uri, redirect must match uuApp base uri or aliased base uri
      let baseUri = requestUri.getBaseUri().toString();
      if (!(redirectUri.startsWith(baseUri) || (baseUriAlias && redirectUri.startsWith(baseUriAlias)))) {
        return false;
      }
    } else {
      // In case of relative uri, path must be within uuApp base uri or aliased base uri path
      let baseUriPath = requestUri.getBaseUri().getRelativeUri().toString();
      let baseUriAliasPath = baseUriAlias && (baseUriAlias.replace(/^[a-z]+:\/\/[^\/]+/, "") || "/");
      if (!(redirectUri.startsWith(baseUriPath) || (baseUriAliasPath && redirectUri.startsWith(baseUriAliasPath)))) {
        return false;
      }
    }
    return true;
