...
class ClientCredentialsHandler {
  ...
  static async generateClientKeyPair() {
    return new Promise((resolve, reject) => {
      let opts = {modulusLength: 2048, publicKeyEncoding: { type: 'spki', format: 'pem' }, privateKeyEncoding: { type: 'pkcs8', format: 'pem' }};
      Crypto.generateKeyPair("rsa", opts, (error, publicKey, privateKey) => error ? reject(error) : resolve({publicKey, privateKey}));
    });
  }
   ...
   * @see ClientCredentialsProvider.getClientCredentials
   */
  static async getUnregisteredCredentials(providerName, awid = null) {
    if (!awid) {
      awid = this._getAwidFromContext();
    }
    if (!awid) {
