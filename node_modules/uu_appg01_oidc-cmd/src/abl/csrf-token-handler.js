...
const logger = LoggerFactory.get("uuapp.oidc.CsrfTokenHandler");
...
class CsrfTokenHandler {
  ...
  static async isCsrfTokenValid(token, uri, session) {
    ...
    if (!expiresAt || !tokenValue || !salt) {
      logger.error("Invalid CSRF token: token incomplete");
      return false;
    }
    if (Number(expiresAt) * 1000 < Date.now()) {
      logger.error("Invalid CSRF token: token expired");
      return false;
    }
    ...
    if (tokenValue !== expectedValue) {
      logger.error("Invalid CSRF token: signature does not match");
      return false;
    }
    return true;
  }
