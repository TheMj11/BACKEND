...
const APP_STATUS_METRICS_PROGRESS_BASE_URI_CONFIG_NAME = "uu_app_status_metrics_progress_base_uri";
const APP_STATUS_PROGRESS_BUS_CODE_CONFIG_NAME = "uu_app_status_progress_bus_code";
const APP_STATUS_METRICS_PROGRESS_BUS_CODE_CONFIG_NAME = "uu_app_status_metrics_progress_bus_code";
...
const APP_STATUS_METRICS_PROGRESS_UPDATE_PERIOD_MS_CONFIG_NAME = "uu_app_status_metrics_progress_update_period_ms";
const APP_STATUS_METRICS_PROGRESS_UPDATE_PERIOD_MS_DEFAULT_VALUE = 60 * 1000;
const APP_STATUS_PROGRESS_DATA_HISTORY_COUNT_CONFIG_NAME = "uu_app_status_progress_data_history_count";
const APP_STATUS_METRICS_PROGRESS_DATA_HISTORY_COUNT_CONFIG_NAME = "uu_app_status_metrics_progress_data_history_count";
...
const APP_STATUS_METRICS_PROGRESS_WARNING_PERIOD_CONFIG_NAME = "uu_app_status_metrics_progress_problem_warning_period";
...
const APP_STATUS_METRICS_PROGRESS_ERROR_PERIOD_CONFIG_NAME = "uu_app_status_metrics_progress_problem_error_period";
...
const APP_STATUS_METRICS_PROGRESS_EXPIRE_PERIOD_CONFIG_NAME = "uu_app_status_metrics_progress_expire_period";
...
const APP_STATUS_METRICS_PROGRESS_ITEMS_MAX_COUNT_CONFIG_NAME = "uu_app_status_metrics_progress_items_max_count";
...
class StatusUpdater {
  constructor() {
    ...
    this.metricsProgressUpdatePeriodMs =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_UPDATE_PERIOD_MS_CONFIG_NAME) ||
      APP_STATUS_METRICS_PROGRESS_UPDATE_PERIOD_MS_DEFAULT_VALUE;
    ...
    this.metricsProgressErrorPeriod =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_ERROR_PERIOD_CONFIG_NAME) || APP_STATUS_PROGRESS_ERROR_PERIOD_DEFAULT_VALUE;
    ...
    this.metricsProgressWarningPeriod =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_WARNING_PERIOD_CONFIG_NAME) ||
      APP_STATUS_PROGRESS_WARNING_PERIOD_DEFAULT_VALUE;
    ...
    this.metricsProgressUri =
      (Config.get(APP_STATUS_METRICS_PROGRESS_BASE_URI_CONFIG_NAME) &&
      UriBuilder.parse(Config.get(APP_STATUS_METRICS_PROGRESS_BASE_URI_CONFIG_NAME))) ||
      this.progressUri;
    ...
    this.metricsProgressBusCode =
      Config.get(APP_STATUS_METRICS_PROGRESS_BUS_CODE_CONFIG_NAME) || `${APP_STATUS}_${Config.get("asid")}_metrics`;
    ...
    this.metricsProgressExpirePeriod =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_EXPIRE_PERIOD_CONFIG_NAME) ||
      APP_STATUS_PROGRESS_EXPIRE_PERIOD_DEFAULT_VALUE;
    ...
    this.metricsProgressItemsMaxCount =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_ITEMS_MAX_COUNT_CONFIG_NAME) ||
      APP_STATUS_PROGRESS_ITEMS_MAX_COUNT_DEFAULT_VALUE;
    ...
    this.metricsProgressCode = this.progressCode + "_metrics";
    ...
    this.metricsProgressDataHistoryCount =
      Config.getNumber(APP_STATUS_METRICS_PROGRESS_DATA_HISTORY_COUNT_CONFIG_NAME) ||
      APP_STATUS_PROGRESS_DATA_HISTORY_COUNT_DEFAULT_VALUE;
   ...
   * @param {Date} [options.atTime] - time for current counter data
   * @param {Array} [options.metrics]
   */
  async update(options = {}) {
    if (!options.atTime) {
      options.atTime = new Date();
    }

    // 1-2. System prepares progress proceedDtoIn
    let proceedDtoIn = this._getProceedDtoIn(options.metrics, options.atTime);

    // 3. System sets progress/proceed useCase
    let progressUri = options.metrics
      ? this.metricsProgressUri.setUseCase(PROGRESS_PROCEED)
    ...
    let session;
    if (this.progressUuEe && !options.metrics) {
      // 5.A
    ...
    try {
      await AppClient.post(progressUri, proceedDtoIn, { session });
    } catch (e) {
      LOGGER.error(`Progress proceed failed: `, e);
      if (e.code && e.code.endsWith(PROGRESS_DOES_NOT_EXIST)) {
        await this._processMissingProgress(proceedDtoIn, !!options.metrics, session);
        ...
        if (e.paramMap && e.paramMap.state && PROGRESS_STATES.includes(e.paramMap.state)) {
          await this.startAndProceedProgress(proceedDtoIn, !!options.metrics, session);
        }
      } else if (e.code && e.code === "uu-appg01/authorization/accessDenied") {
        this._accessDeniedLogger(proceedDtoIn.code, progressUri, session);
      }
    }
  }
   ...
   */
  _getProceedDtoIn(metrics, atTime = new Date()) {
    // 1. System sets estimatedNextProgressTs, expireAt
    const date = new Date();
    const estimatedNextProgressTs = metrics
      ? new Date(date.getTime() + this.metricsProgressUpdatePeriodMs)
      : new Date(date.getTime() + this.progressUpdatePeriod * 1000)
    const expireAt = metrics
      ? new Date(date.getTime() + this.metricsProgressExpirePeriod * 1000)
      : new Date(date.getTime() + this.progressExpirePeriod * 1000);
    ...
    if (metrics) {
      ...
      let componentData = AppStatusCounter.getData(atTime);
      let dataHistoryKey = componentData.dataHistoryKey;
      delete componentData.dataHistoryKey;
      proceedDtoIn.dataHistoryKey = dataHistoryKey;
      data = {
    ...
    proceedDtoIn = {
      ...proceedDtoIn,
      code: metrics ? this.metricsProgressCode : this.progressCode,
      estimatedNextProgressTs: estimatedNextProgressTs.toISOString(),
      lockSecret: LOCKSECRET,
      delayWarning: metrics ? this.metricsProgressWarningPeriod : this.progressWarningPeriod,
      delayError: metrics ? this.metricsProgressErrorPeriod : this.progressErrorPeriod,
  ...
  async _getProgressBusCreateDtoIn(isMetricsProgress, subAppInstance) {
    const authorizationStrategy = await this._getAuthorizationStategy();
    return {
      code: isMetricsProgress ? this.metricsProgressBusCode : this.progressBusCode,
      name: subAppInstance.name || this.uuSubAppName,
      desc: this.asid,
      maxNumberOfProgressItems: isMetricsProgress ? this.metricsProgressItemsMaxCount : this.progressItemsMaxCount,
      dataHistoryMaxSize: isMetricsProgress ? this.metricsProgressDataHistoryCount : this.progressDataHistoryCount,
      data: {
        uuSubAppName: this.uuSubAppName,
        uuSubAppVersion: APPINFO.uuSubAppVersion,
        asid: this.asid,
  ...
  async _processMissingProgress(proceedDtoIn, isMetricsProgress, session) {
    // 6.B.1.A.
    ...
    let progressCreateDtoIn = {
      ...
      progressBusCode: isMetricsProgress ? this.metricsProgressBusCode : this.progressBusCode,
      name: `<uu5string/>${uniqueIdLastSix}${this.spp === SPP_DEFAULT ? "" : `.${this.spp}`}<br/>[${appVersion}]`,
      dataHistoryMaxSize: 10,
    };

    await this._checkExistingNode(session, isMetricsProgress, nodeNumberInNodeSet, nodeSetUniqueId);
    await this._createProgress(progressCreateDtoIn, isMetricsProgress, session);
    // 6.B.2.A.6...
    await this.startAndProceedProgress(proceedDtoIn, isMetricsProgress, session);
  }

  async _checkExistingNode(session, isMetricsProgress, nodeNumberInNodeSet, nodeSetUniqueId) {
    ...
    if (nodeNumberInNodeSet === "1") {
      ...
      let progressUri = isMetricsProgress ? this.metricsProgressUri : this.progressUri;
      let progressBusCode = isMetricsProgress ? this.metricsProgressBusCode : this.progressBusCode;
      try {
        // 6.B.2.A.3.A.1.A.
        // 6.B.2.A.3.A.1.A.1. System sets use case to progressBus/load
        progressUri.setUseCase(PROGRESS_BUS_LOAD);
        // 6.B.2.A.3.A.1.A.2. System loads progressBus using AppClient with session from HDS 4.
        await AppClient.get(progressUri, { code: progressBusCode }, { session });
      } catch (e) {
        // 6.B.2.A.3.A.1.B.
        ...
        if (e.code && e.code.endsWith(PROGRESS_BUS_DOES_NOT_EXIST)) {
          ...
          let progressBusCreateDtoIn = await this._getProgressBusCreateDtoIn(isMetricsProgress, subAppInstance);

          await this._createProgressBus(progressBusCreateDtoIn, progressUri, session);
        } else {
          // 6.B.2.A.3.A.1.B.1.B.
  ...
  async startAndProceedProgress(proceedDtoIn, isMetricsProgress, session) {
    let progressUri = isMetricsProgress ? this.metricsProgressUri : this.progressUri;

    // 6.B.2.A.6. System sets progress/start use case
    progressUri.setUseCase(PROGRESS_START);
    ...
    try {
      // 6.B.2.A.8.A.
      await AppClient.post(progressUri, progressStartDtoIn, { session });
    } catch (e) {
      // 6.B.2.A.8.B.
    ...
    progressUri.setUseCase(PROGRESS_PROCEED);
  ...
  async _createProgressBus(progressBusCreateDtoIn, progressUri, session) {
    // 6.B.2.A.3.A.1.B.1.A.2. System sets use case to progressBus/create
    progressUri.setUseCase(PROGRESS_BUS_CREATE);

    // 6.B.2.A.3.A.1.B.1.A.3. System tries to create progressBus
    try {
      // 6.B.2.A.3.A.1.B.1.A.3.A.
      await AppClient.post(progressUri, progressBusCreateDtoIn, { session });
    } catch (e) {
  ...
  async _createProgress(progressCreateDtoIn, isMetricsProgress, session) {
    let progressUri = isMetricsProgress ? this.metricsProgressUri : this.progressUri;
    // 6.B.2.A.4. System sets progress/create use case
    progressUri.setUseCase(PROGRESS_CREATE);

    // 6.B.2.A.5. System tries to create new progress
    try {
      // 6.B.2.A.6.A.
      await AppClient.post(progressUri, progressCreateDtoIn, { session });
    } catch (e) {
      // 6.B.2.A.6.B.
  ...
  _accessDeniedLogger(progressCode, progressUri, session) {
    LOGGER.error(
  ...
  getAppStatusMetricsProgressBaseUriConfigName() {
    return APP_STATUS_METRICS_PROGRESS_BASE_URI_CONFIG_NAME;
  }
