...
const OIDC_PROVIDER_URI = "https://uuapp-dev.plus4u.net/uu-oidc-maing02/eca71064ecce44b0a25ce940eb8f053d/oidc";

module.exports = async function(opts) {
  let { stack, name, namespace, template, content, hasServer, hasClient, skipInstall } = opts || {};
  ...
  try {
    ...
    let namespaceAppCamelCase = namespaceApp.charAt(0).toLowerCase() + namespaceApp.substring(1); // uu_demoappg01_main-client => uuDemoapp
    ...
    let context = {
      ...
      oidcProviderUri: OIDC_PROVIDER_URI,
      valuePlaceholder: VALUE_PLACEHOLDER,
      insomniaContextPath: name.replace(/_/g,"-").split("-server")[0],
      errorCode: namespaceAllParts.join("-"),
      workspaces: [], 
      devkitVersion: toolsVersion,
      cloudDevkitVersion: CLOUD_DEVKIT_VERSION,
      uu5devkitVersion: UU5DEVKIT_VERSION,
      secretStoreMasterKey1: generateMasterKey(64),
      secretStoreMasterKey2: generateMasterKey(64)
    };

    let isLib = type.match(/(?<!uuscript)lib$/) || type.endsWith("devkit");

    let templatesDir = path.join(__dirname, "templates");
    let templateType = template === "simple" ? type.replace(/^(nodejs|uu5)-(app)$/g, (match, g1, g2) => `${g1}-simple-${g2}`) : type;
    let typeDir = path.join(templatesDir, "__" + (type === "nodejs-app" && !hasClient ? "nodejs-service" : templateType));
    ...
    const RENAME_MAP = {
      ...
      [path.join("app", "api", "warnings", "project-use-case-warning.js")]: path.join("app", "api", "warnings", `${projectClassPrefix}-use-case-warning.js`),
      [path.join("app", "api", "warnings", "project-warning.js")]: path.join("app", "api", "warnings", `${projectClassPrefix}-warning.js`),
      ...
      [path.join("src", "project-client.js")]: path.join("src", `${projectClassPrefix}-client.js`),
      [path.join("bin", "project.js")]: path.join("bin", `${name}.js`)
    };
    if (template !== "simple") {
      RENAME_MAP[path.join("app", "components", "project-client.js")] = path.join("app", "components", `${projectClassPrefix}-client.js`);
      RENAME_MAP[path.join("app", "constants", "project-constants.js")] = path.join("app", "constants", `${projectClassPrefix}-constants.js`);
      RENAME_MAP[path.join("app", "constants", "project-states.js")] = path.join("app", "constants", `${projectClassPrefix}-states.js`);
    }

    let filesForParentDir = [".editorconfig", "README.md", "gitignore", "gitattributes"];
    if (isLib) {
      filesForParentDir.push("LICENSE");
      filesForParentDir.push(".pnpmfile.cjs");
    }
    ...
    if (devkit.development.uuAppBoxRegistryBaseUri === undefined) {
      devkit.development.uuAppBoxRegistryBaseUri = VALUE_PLACEHOLDER;
    }
    ...
    if (type === "nodejs-app" || type === "nodejs-service") {
      ...
      if (template === "simple") {
        ...
        // Rename abl subfolder
        fs.renameSync(path.join(".", "app", "abl", "project"), path.join(".", "app", "abl", projectClassPrefix));
        // Rename metamodel file
        fs.renameSync(
          path.join(".", "app", "config", "metamodel", "uu-businessterritory-maing01", "metamodel.json"),
          path.join(".", "app", "config", "metamodel", "uu-businessterritory-maing01", `${context.insomniaContextPath}-metamodel.json`)
        );
      }
    } else if (type === "nodejs-uuscriptlib") {
      // Rename src subfolder
      fs.renameSync(path.join(".", "src", "project"), path.join(".", "src", projectClassPrefix));
      fs.renameSync(path.join(".", "test", "project"), path.join(".", "test", projectClassPrefix));
