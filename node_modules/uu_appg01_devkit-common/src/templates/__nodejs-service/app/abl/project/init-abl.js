...
const logger = LoggerFactory.get("<%= context.projectClassName%>.InitAbl");
...
class InitAbl {
  ...
  async init(uri, dtoIn, session) {
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      let awscId;
      try {
        const awscDtoOut = await AppClient.post(awscCreateUri, createAwscDtoIn, { session: sysIdentitySession });
