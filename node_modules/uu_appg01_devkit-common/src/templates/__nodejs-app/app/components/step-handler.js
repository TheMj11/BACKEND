...
const <%= context.projectClassName %>UseCaseWarning = require("../api/warnings/<%= context.projectClassFile %>-use-case-warning");
  ...
  async storeStep(uuObject, step, progressRequired = true) {
    ...
    if (this.progressClient || progressRequired) {
      try {
        ...
        if (!progressRequired) {
          DtoBuilder.addWarning(new <%= context.projectClassName %>UseCaseWarning(e.code, e.message, e.parameters));
        } else {
          throw e;
        }
      }
