...
class Validator {
  ...
  validateCustom(uri, dtoIn, validationType, error, uuAppErrorMap) {
    uuAppErrorMap ??= UseCaseContext.getAttribute(<%= context.projectClassName %>Constants.UU_APP_ERROR_MAP);
  ...
  _getCode(uri) {
    return `${<%= context.projectClassName %>Constants.ERROR_PREFIX}/${uri.getUseCase()}/${WARNING_CODE}`;
  }
}
