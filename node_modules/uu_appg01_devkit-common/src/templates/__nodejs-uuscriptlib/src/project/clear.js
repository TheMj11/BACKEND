...
const Names = {
  ...
  SCRIPT_NAME: "<%=  context.projectClassName %>Clear",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class <%=  context.projectClassName %>ClearClient {
  ...
  async clearFinalize() {
    return this.<%=  context.projectClassName.charAt(0).toLowerCase() + context.projectClassName.substr(1) %>Client.clearFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const script<%=  context.projectClassName %>ClearDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("script<%=  context.projectClassName %>ClearDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script <%=  context.namespaceAppCamelCase %> clear started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new <%=  context.projectClassName %>ClearClient(dtoIn.lockSecret);
  ...
  <%=  context.namespaceAppCamelCase %> = await mainContext.clearFinalize();
