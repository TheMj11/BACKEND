...
const Names = {
  ...
  SCRIPT_NAME: "<%=  context.projectClassName %>InitRollback",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class <%=  context.projectClassName %>InitRollbackClient {
  ...
  async initFinalizeRollback() {
    return this.<%=  context.projectClassName.charAt(0).toLowerCase() + context.projectClassName.substr(1) %>Client.initFinalizeRollback(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const script<%=  context.projectClassName %>InitRollbackDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("script<%=  context.projectClassName %>InitRollbackDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script <%=  context.namespaceAppCamelCase %> initRollback started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new <%=  context.projectClassName %>InitRollbackClient(dtoIn.lockSecret);
  ...
  <%=  context.namespaceAppCamelCase %> = await mainContext.initFinalizeRollback();
