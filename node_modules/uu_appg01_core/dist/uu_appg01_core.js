...
if (typeof window == "undefined") {
  var path = __webpack_require__("?368b");
  var fs = __webpack_require__("?f29d");
}
...
class LoggerFactory {
  ...
  _getModuleName(modulePath) {
    ...
    if (packageJsonPath) {
      ...
      let packageJson;
      try {
        packageJson = __webpack_require__("../../node_modules/uu_appg01_core-logging/src sync recursive")(packageJsonPath);
      } catch (e) {
  ...
  _nameFromParts(parts) {
    return parts.map(part => part.replace(/[^a-zA-Z0-9_-]/g, "-")).map(part => part.replace(/[_-]+([a-zA-Z0-9])?/g, (_, p1) => p1 ? p1.toUpperCase() : "")).map(part => part.split(/[\.]/g)).flat().filter(part => part).map(part => part[0].toUpperCase() + part.slice(1)).join(".");
  }
}
...
class Perflog {
  ...
  static measureSection(name = null, attributes = {}, fn) {
    ...
    let sectionWrapper = async () => {
      ...
      let sectionId = new SectionId(parentId.getTraceId(), parentId.getId(), -1);
...
// These parameters are not parsed by UriParser,
// but must be part of the validation as they are contained in valid URIs.
const PATH_SEGMENT_PARAMS = `(?:;${UNRESERVED_ENCODED}*(?:=${UNRESERVED_ENCODED}+)?)*`;
...
const URI_REGEXP = new RegExp(`^(?:${GATEWAY})?\\/${PRODUCT}\\/${WORKSPACE}(?:/${USE_CASE})?${PATH_SEGMENT_PARAMS}\\/?(?:\\?${PARAMETERS}?)?$`);
...
class UriParser {
  ...
  parseGateway(value) {
    ...
    let result = value.replace(/\/$/, "").match(GATEWAY_REGEXP);
...
/***/ "../../node_modules/uu_appg01_core-utils/src/async-executor.js":
/***/ ((module) => {
...
/***/ "../../node_modules/uu_appg01_core-utils/src/cache.js":
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {
...
class Cache {
  ...
  _storeToStorage() {
    if (localStorageAvailable && this._name && !this._isContentStoreScheduled) {
      if (!this._nextWrite) {
        // use microtask for the first time
        queueMicrotask(() => {
  ...
  load(failOnError = false) {
    ...
    try {
      if (this._fileSrc !== undefined && this._fileSrc) {
        ...
        let data = ({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""})[this._senvSrc] || "";
        this._data = this._parse(data);
      } else if (this._menvSrc !== undefined && this._menvSrc) {
        loadedSrc = this._menvSrc;
        let regex = this._menvSrc;
        let envKeys = Object.keys(({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""}));
        let result = {};
        envKeys.forEach(function (key) {
          if (regex.test(key)) {
            result[key.toLowerCase()] = ({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""})[key];
          }
        });
        this._data = result;
      }
...
class Config {
  ...
  static activateProfiles(...profiles) {
    ...
    if (typeof window == "undefined") {
      ...
      let cfgDir = ({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""})["CONFIG_DIR"] || Config._getDefaultConfigDir();
      let sysCfgDir = ({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""})["SYS_CONFIG_DIR"] || Config._getDefaultSysConfigDir();
  ...
  static _getChain() {
    ...
    if (!chain) {
      ...
      if (actProf.length === 0) {
        let envProfiles = ({"NAME":"uu_appg01_core","VERSION":"6.2.0","NAMESPACE":"UuApp","OUTPUT_NAME":"uu_appg01_core","NODE_ENV":(typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development"),"TARGET_ENVIRONMENT":"browser","MOCK_DATA_BASE_URI":""})["CONFIG_PROFILE"] || (typeof process !== "undefined" && process.env && (process.env.NODE_ENV) || "development");
        if (envProfiles) {
...
module.exports = {
  ...
  get Cache() {
    return __webpack_require__("../../node_modules/uu_appg01_core-utils/src/cache.js");
  },
  get LruCache() {
  ...
  get ConfigFilesLoader() {
    return __webpack_require__("?9e54");
  },
  get ResolvablePromise() {
    return __webpack_require__("../../node_modules/uu_appg01_core-utils/src/resolvable-promise.js");
  },
  get AsyncExecutor() {
    return __webpack_require__("../../node_modules/uu_appg01_core-utils/src/async-executor.js");
  }
};
...
const Cache = __webpack_require__("../../node_modules/uu_appg01_core-utils/src/cache.js");
...
class Mutex {
  ...
  static async sleep(ms) {
    ...
    return new Promise(resolve => {
      setTimeout(() => resolve(Date.now() - initiated), ms).unref();
    });
  }
  /**
...
/***/ "../../node_modules/uu_appg01_core-utils/src/resolvable-promise.js":
/***/ ((module) => {
...
var floatingVersion = "/6.0.0/";
if (uri.substr(-floatingVersion.length) !== floatingVersion) floatingVersion = "/6.x/";
if (uri.substr(-floatingVersion.length) === floatingVersion) uri = uri.substr(0, uri.length - floatingVersion.length) + "/6.2.0/";
...
/***/ "../../node_modules/uu_appg01_core-logging/src sync recursive":
/***/ ((module) => {
...
webpackEmptyContext.id = "../../node_modules/uu_appg01_core-logging/src sync recursive";
