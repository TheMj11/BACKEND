...
class CacheConfigLoader {
  ...
  loadConfig() {
    ...
    const initialConfig = mergeWithOverwrite(
      /** @type {{data: Partial<CacheConfigMap>}[]} */
  ...
  getCacheConfig(cacheCode, disableHardwiredHandlers = false) {
    ...
    if (!disableHardwiredHandlers) {
      return this.addHardwiredHandlers(cacheConfig);
    }
  ...
  /**
   * Adds hardwired handlers to the cache config if they are not disabled using the `disableHardwiredHandlers` option either in cache or as an argument.
   * @param {CacheConfig} cacheConfig
   * @returns {CacheConfig}
   */
  addHardwiredHandlers(cacheConfig) {
    if (!cacheConfig.config.disableHardwiredDefaults) {
      const handlers = [...HARDWIRED_HANDLERS_PRE, ...cacheConfig.handlers, ...HARDWIRED_HANDLERS_POST];
      cacheConfig = mergeWithOverwrite([cacheConfig, /** @type {Partial<CacheConfig>} */ ({ handlers })]);
    }
    return cacheConfig;
  }
  ...
  getDefaultCacheConfig() {
    return mergeWithOverwrite(
      /** @type {Partial<CacheConfig>[]} */ ([{}, CACHE_DEFINITION_DEFAULTS, CACHE_MANAGER_DEFINITION_DEFAULTS])
    );
  }
  ...
  _getConfigValue(key, type, recursive = false) {
    ...
    switch (type) {
      ...
      default: {
        const [, arrayType] = arrayRegex.exec(type) || [];
        if (arrayType) {
          return Config.getArray(key, arrayType, recursive);
        }
   ...
   */
  loadOverrides(cacheCode, config, key, type, defaultValue) {
