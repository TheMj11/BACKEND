...
const AUTHN_CONSTRAINTS_CACHE_SIZE = 256;
const AUTHN_CONSTRAINTS_CACHE_STORAGE_KEY = "uu_app_oidc_cache_authn_constraints";
const AUTHN_CONSTRAINTS_CACHE = new LruCache({
  maxSize: AUTHN_CONSTRAINTS_CACHE_SIZE,
  localStorageKey: AUTHN_CONSTRAINTS_CACHE_STORAGE_KEY
});
...
class AuthenticationService {
  ...
  /**
   * Returns whether the user (global) session of primary authentication service is untrusted (does not meet acr or max age criteria).
   * @returns {boolean} Whether the user (global) session of primary authentication service is untrusted (does not meet acr or max age criteria).
   */
  static isSessionUntrusted() {
    let primaryProvider = this._providers[this._primaryProvider];
    return primaryProvider ? primaryProvider.isSessionUntrusted() : null;
  }
  ...
  async restoreSession(options = {}) {
    ...
    if (!this._restoreSessionPromise) {
      ...
      this._clientId = await ClientCredentialsProvider.getClientId(this._serviceName);
      let session = BrowserSession.loadFromCache(this, this._clientId);
      let authnConstraints = this._loadAuthnConstraints();
      acrValues ||= authnConstraints.acrValues || this._defaultAcrValues;
      maxAge ||= authnConstraints.maxAge || this._defaultMaxAge;
      let restoreSessionFn;
      let restoreSessionArgs;
      if (session && session.matches(acrValues, maxAge)) {
      ...
      this._restoreSessionPromise = restoreSessionFn(restoreSessionArgs).then(r => {
        ...
        if (e && e.message && e.message.match(/not satisfied/i)) {
          this._sessionUntrusted = true;
        } else {
          this._sessionUntrusted = false;
        }
        this._isRestoringSession = false;
  ...
  async authenticate(options, scope = null) {
    ...
    return this._runningAuthnPromise = (async () => {
      try {
        if (!this._oauthClient) {
          await this._loadOauthClientPromise;
        }
        ...
        let scopeMode = options.scopeMode || this._defaultScopeMode;
        if ((!scope || scope.length == 0) && scopeMode !== "none") {
          scope = Dom.canonicalAppBaseUri || Dom.appBaseUri;
          scope = scope.replace(/\/$/, "");
        }
        let authnConstraints = this._loadAuthnConstraints();
        let currentUuIdentity = this._currentSession?.getIdentity()?.getUuIdentity();
        if (!options.loginHint || options.loginHint === currentUuIdentity) {
          // Apply stored constraint only if we are not trying to switch user
          if (!options.acrValues && authnConstraints.acrValues) {
            options.acrValues = authnConstraints.acrValues;
          }
          if (!options.maxAge && authnConstraints.maxAge) {
            options.maxAge = authnConstraints.maxAge;
          }
        }
        let result = await flow.authenticate(options, scope);
        ...
        this._storeAuthnConstraints(authnConstraints, session);
  ...
  /**
   * Returns whether the user (global) session is untrusted (does not meet acr or max age criteria).
   * @returns {boolean} Whether the user (global) session is untrusted (does not meet acr or max age criteria).
   */
  isSessionUntrusted() {
    return this._sessionUntrusted;
  }
  ...
  async _loadOauthClient(serviceName, serviceUri, opts) {
    this._clientId = await ClientCredentialsProvider.getClientId(serviceName);
  ...
  _initAutoLogoutSupport() {
    ...
    this.addListener("sessionChanged", event => {
      ...
      // plan local logout
      let expiresAt;
      let maxAge = this._loadAuthnConstraints().maxAge;
      if (maxAge) {
        expiresAt = (this._currentSession.getAttribute("auth_time") + maxAge) * 1000;
      } else {
        expiresAt = this._currentSession.getExpiresAt() || 1000 * (this._currentSession.getAttribute("exp") || 0);
      }
      if (!expiresAt) return;
      let now = Date.now();
  ...
  _initSessionExtendedAndExpiringSupport() {
    ...
    this.addListener("sessionChanged", e => {
      ...
      let delay = expiresAt - now - sessionExpiringNotificationTime * 1000;
      if (delay < (expiresAt - now) / 2) {
        delay = Math.max(0, (expiresAt - now) / 2);
      }
  ...
  async _initSessionStateChangeNotifier() {
    ...
    if (sessionCheckInterval && (await this._authnFlow.isCheckSessionSupported())) {
      this._sessionChecker = new SessionStateChangeNotifier(this._authnFlow, sessionCheckInterval, this._clientId, () => this._currentSession.getState());
  ...
  async _doAuthenticateRefresh() {
    ...
    try {
      ...
      let authnConstraints = this._loadAuthnConstraints();
      let scope = authnConstraints.scope;
      let acrValues = authnConstraints.acrValues;
      let maxAge = authnConstraints.maxAge;
      let result;
  ...
  _storeAuthnConstraints(constraints, session) {
    let cacheKey = Digest.sha1(`${this._serviceName}|${Dom.appBaseUri}|${this._clientId}`);
    let expiresAt = session?.isAuthenticated() && session.getAttribute("exp") * 1000 || 0;
    let maxAge = expiresAt > Date.now() ? expiresAt - Date.now() : 0;
    AUTHN_CONSTRAINTS_CACHE.set(cacheKey, constraints, maxAge);
  }
  _loadAuthnConstraints() {
    let cacheKey = Digest.sha1(`${this._serviceName}|${Dom.appBaseUri}|${this._clientId}`);
    return AUTHN_CONSTRAINTS_CACHE.get(cacheKey) || {};
  }
  _removeAuthnConstraints() {
    AUTHN_CONSTRAINTS_CACHE.clear();
  }
}
...
class BrowserSession {
  static from(session, authenticationContext, expiresAt, state, authnService) {
    ...
    session.close = async function () {
      ...
      session._authnService._removeAuthnConstraints();
    }.bind(session);
    ...
    session.storeToCache = function (clientId = null) {
      ...
      storeKey += `_${Digest.sha1(`${clientId}@${Dom.appBaseUri}`)}`;
...
class ExternalCallTokenProvider {
  ...
  constructor(currentSession, callUri) {
    ...
    this._authnService = currentSession._authnService;
    ...
    if (!this._isValid(sub, authTime)) {
      if (!this._runningPromise) {
        ...
        let {
          acrValues,
          maxAge
        } = this._authnService._loadAuthnConstraints();
        let authOpts = {
          prompt: "none",
          usePopup: false,
          acrValues,
        ...
        if (this._authnService._authnFlow.getCallToken) {
          this._runningPromise = this._authnService._authnFlow.getCallToken(authOpts, this._callUri);
        } else {
          this._runningPromise = this._authnService._authnFlow.authenticate(authOpts, this._callUri);
        }
      }
      try {
        let {
...
class Session extends CoreSession {
  ...
  async getCallToken(scope, opts = {}) {
    ...
    if (!useTokenExchange) {
      ...
      let tokenKey;
      if (Array.isArray(scope)) {
        tokenKey = Digest.sha1(`${this.getIdentity().getUuIdentity()}:${this.getAuthenticationId()}:${this.getAttribute(JWT_CLAIM_AUTH_LEVEL_OF_ASSURANCE)}:${clientId}:${scope.sort().join("|")}`);
      } else {
        tokenKey = Digest.sha1(`${this.getIdentity().getUuIdentity()}:${this.getAuthenticationId()}:${this.getAttribute(JWT_CLAIM_AUTH_LEVEL_OF_ASSURANCE)}:${clientId}:${scope}`);
      }
  ...
  async actAs(uuIdentity, scope = null, opts = {}) {
    ...
    let tokenKey;
    if (Array.isArray(scope)) {
      tokenKey = Digest.sha1(`${this.getIdentity().getUuIdentity()}:${this.getAuthenticationId()}:${this.getAttribute(JWT_CLAIM_AUTH_LEVEL_OF_ASSURANCE)}:${uuIdentity}:${clientId}:${scope.sort().join("|")}`);
    } else {
      tokenKey = Digest.sha1(`${this.getIdentity().getUuIdentity()}:${this.getAuthenticationId()}:${this.getAttribute(JWT_CLAIM_AUTH_LEVEL_OF_ASSURANCE)}:${uuIdentity}:${clientId}:${scope}`);
    }
    let grantedToken = await this._oauthClient.getToken(tokenKey);
    let credentials = await ClientCredentialsProvider.getClientCredentials(this._serviceName);
    if (!grantedToken) {
  ...
  // TODO Temorary fix of method from core session, can be removed once fixed there.
...
if (uri.substr(-floatingVersion.length) !== floatingVersion) floatingVersion = "/4.x/";
if (uri.substr(-floatingVersion.length) === floatingVersion) uri = uri.substr(0, uri.length - floatingVersion.length) + "/4.0.1/";
...
/******/ 			return "chunks/uu_appg01_oidc/" + chunkId + "-" + "5730cced754b013fa36a" + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "uu_appg01_oidc_4_0_1:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = this["__webpack_jsonp_uu_appg01_oidc_4_0_1_uu_appg01_oidc"] = this["__webpack_jsonp_uu_appg01_oidc_4_0_1_uu_appg01_oidc"] || [];
