...
yargs
  ...
  .command(
    "uuLogStoreExport [tokenFile]",
    "Export logs of the application.",
    {}, getScript("uuLogStoreExport"),
    [() => {
      console.warn(
        require("chalk").yellow("WARNING") +
        " This task is deprecated, please use the uuCloudg02LogExport task. Visit https://uuapp.plus4u.net/uu-bookkit-maing01/e884539c8511447a977c7ff070e7f2cf/book/page?code=95381158 for more information."
      );
    }],
    true
  )
  ...
  .command("createUuCommnadLineApiClient", "Generate uuCommandLineApi", {}, getScript("generateCommandLineApi"))
  .command("createUuCommandLineApiClient", "Generate uuCommandLineApi", {}, getScript("generateCommandLineApi"))
    ...
    try {
      ...
      if (!process.env.UU_APPG01_DEVKIT_SPECVC) {
        ...
        if (scriptName !== "uuAppDevKitUpgrade") {
          let { specList } = helpers.getTemplateInfo();
          let spec = specList.find((it) => it.name === "uu_appg01_devkit");
          if (spec?.version) {
            let myVersion = require("../package.json").version;
            if (semver.valid(spec.version) && semver.compare(spec.version, myVersion) > 0) {
              console.warn(
                require("chalk").yellow("WARNING") +
                  ` This project was already upgraded to uu_appg01_devkit ${spec.version}, you have ${myVersion}. Install newer devkit to prevent incompatibility issues.`,
              );
            }
          }
        }
