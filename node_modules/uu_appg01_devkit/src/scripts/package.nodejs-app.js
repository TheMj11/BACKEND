...
module.exports = class Package {
  ...
  async _createMetaModelZipArchieves(uuCloudConfig) {
    const metaModelFileName = `${this.config.get("product").replaceAll("_", "-")}-metamodel.json`;
    ...
    const metamodelVersion = this._resolveMetamodelVersion(metaModelRootDir, metaModelFileName);
    if (!metamodelVersion) {
      console.log(
        "Warning: Failed to resolve meta model version.\n" +
        `  Expected ${metaModelFileName} file containing version attributte embedded in ${metaModelRootDir} direcotry.`
      );
    }

    const metamodelVersionPostfix = metamodelVersion ? `_${metamodelVersion}` : "";
    const uuBtMetaModelZip = path.resolve(
      metaModelRootDir,
      "..",
      `metamodel_uu-businessterritory-maing01${metamodelVersionPostfix}.zip`
    );
    const uuMtMetaModelZip = path.resolve(
      metaModelRootDir,
      "..",
      `metamodel_uu-myterritory-maing01${metamodelVersionPostfix}.zip`
    );
  ...
  _resolveMetamodelVersion(dir, fileName) {
    const files = fs.readdirSync(dir);
    for (const file of files) {
      const filePath = path.join(dir, file);
      const fileStat = fs.statSync(filePath);
      if (fileStat.isDirectory()) {
        return this._resolveMetamodelVersion(filePath, fileName);
