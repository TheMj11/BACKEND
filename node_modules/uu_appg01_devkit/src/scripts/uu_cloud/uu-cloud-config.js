...
class UuCloudConfig {
  ...
  uuAppBoxConfig() {
    ...
    result.uuBtMetaModelZip = this._getMetaModelZip("metamodel_uu-businessterritory-maing01");
    result.uuMtMetaModelZip = this._getMetaModelZip("metamodel_uu-myterritory-maing01");
  ...
  _getMetaModelZip(fileName) {
    const configDirPath = path.resolve(this.projectRoot, "app/config");
    let configFiles;
    try {
      configFiles = fs.readdirSync(configDirPath);
    } catch(e) {
      if (e.code === "ENOENT") {
        return null;
      } else {
    ...
    const metaModelZipRegexp = new RegExp(`${fileName}(_(([0-9]+\\.){2}[0-9]+).*)?.zip`);
    const metaModelFiles = configFiles.filter(fileName => metaModelZipRegexp.test(fileName));
    if (!metaModelFiles.length) {
      return null;
    } else if (metaModelFiles.length === 1) {
      return path.resolve(configDirPath, metaModelFiles[0]);
    }

    let metaModelZip;
    let metaModelLatestVersionParts = ["0", "0", "0"];
    metaModelFiles.forEach(file => {
      let metaModelVersionParts = file
        .replace(`${fileName}_`, "")
        .replace(".zip", "")
        .split(".");
      if (metaModelVersionParts.length < 3) {
        return;
      }
      if (metaModelVersionParts.length > 3) {
        metaModelVersionParts = [
          ...metaModelVersionParts.slice(0,2),
          metaModelVersionParts.slice(2).join(".")
        ];
      }
      if (
        metaModelVersionParts[0] >= metaModelLatestVersionParts[0] &&
        metaModelVersionParts[1] >= metaModelLatestVersionParts[1] &&
        metaModelVersionParts[2] >= metaModelLatestVersionParts[2]
      ) {
        metaModelLatestVersionParts = [...metaModelVersionParts];
        metaModelZip = file;
      }
    });

    return metaModelZip && path.resolve(configDirPath, metaModelZip);
  }
