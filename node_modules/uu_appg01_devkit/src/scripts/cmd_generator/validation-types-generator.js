...
class ValidationTypesGenerator {
  ...
  constructor(context, executionQueue) {
    ...
    this.validationTypeList = ctx.validationTypeList;
  ...
  async execute(onlyCreateFile = false) {
    ...
    if (onlyCreateFile) {
      ...
      await this._updateValidationTypes();
    }

  }

  async _updateValidationTypes() {
    ...
    if (await fileExists(this.validationTypesPath)) {
      ...
      if (Array.isArray(this.validationTypeList)) {
        if (this.validationType && !this.validationTypeList.includes(this.validationType)) {
          this.validationTypeList.push(this.validationType);
        }
      } else {
        this.validationTypeList = this.validationType ? [this.validationType] : [];
      }
      for (let konst of parsedSourceCode.body) {
        try {
          let constName = konst.declarations[0].id.name;
          if (this.validationTypeList.includes(constName)) {
