...
/*@@viewOn:errors*/
const Errors = {
  ERROR_PREFIX: "<%= context.scriptLibCode%>/<%= context.useCasePascalCase%>Rollback/",
...
const DTO_IN_SCHEMA_TYPE = "script<%= context.useCasePascalCase%>RollbackDtoInType";
const DTO_IN_SCHEMA = `
...
async function main() {
  ...
  // n. call rollback finalize uuCmd
  const appClient = new AppClient({ baseUri: dtoIn.baseUri, session });
  await appClient.cmdPost("<%= context.finalizeRollbackUseCase%>", { lockSecret: dtoIn.lockSecret });
