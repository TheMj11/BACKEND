...
<html lang="en" class="notranslate" translate="no">
  <head>
    ...
    <title><%= context.appName %> apiConsole</title>
    ...
    <meta name="apple-mobile-web-app-title" content="<%= context.appName %>">
    <meta name="application-name" content="<%= context.appName %>">
...
<%='<'%>%=
  ...
  htmlWebpackPlugin.options.renderTemplate("index-error.html", {
    handlerName: "handleApiConsoleScriptError",
    targetElementId: "apiConsole"
  })
<%= '%' %>>

    <script>
      if (!window.Uu5Loader) handleApiConsoleScriptError(new Error("Uu5Loader failed to load."));
      else Uu5Loader.import("./api-console.js").then(function (Index) {
        Index.render("apiConsole");
      }).catch(handleApiConsoleScriptError);
    </script>
