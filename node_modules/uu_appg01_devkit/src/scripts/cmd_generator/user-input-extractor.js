...
class UserInputExtractor {
  ...
  async extractStatic(commandLineArgs) {
    ...
    // privileged
    result.isPrivileged = !!commandLineArgs.privileged;
    ...
    // two-phase cmd
    result.isTwoPhase = !!commandLineArgs.twoPhase;
  ...
  async extractInteractive() {
    ...
    result.isTwoPhase = await this._getUserInput("two-phase? - one of yes, no [no]: ", YES_NO_REGEXP, val => val === "yes");
