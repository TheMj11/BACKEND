...
const { evalTemplate } = require("uu_appg01_devkit-common/src/tools/template-helpers.js");
...
const ScriptGenerator = require("./cmd_generator/script-generator");
...
module.exports = class GenerateCmd {
  ...
  async process() {
    ...
    if (context.isTwoPhase) {
      let validationTypeBase = context.useCase.replace(/\/+./g, m => m.charAt(m.length - 1).toUpperCase());
      let cmdValidationType = `${validationTypeBase}DtoInType`;
      let finalizeCmdValidationType = `${validationTypeBase}FinalizeDtoInType`;
      let finalizeRollbackCmdValidationType = `${validationTypeBase}FinalizeRollbackDtoInType`;
      context.validationTypeList = [cmdValidationType, finalizeCmdValidationType, finalizeRollbackCmdValidationType];
      context.validations = (await evalTemplate(
        path.join(__dirname, "cmd_generator", "templates", "validation-types-two-phase-cmd.js"),
        { cmdValidationType, finalizeCmdValidationType, finalizeRollbackCmdValidationType }
      )).replace(/\r\n/g, "\n");
    }
    ...
    await new ValidationTypesGenerator(context, executionQueue).execute(!context.isTwoPhase);
    if (context.daoStub) {
    ...
    if (context.isTwoPhase) {
      await new ScriptGenerator(context, executionQueue).execute();
    }
  ...
  getContext(userInputContext) {
    ...
    let controllerParams = userInputContext.isTwoPhase ? ["uri"] : (userInputContext.isPrivileged ? ["asid"] : ["awid"]);
    ...
    return {
      ...
      isTwoPhase: userInputContext.isTwoPhase,
      controllerParams,
