...
module.exports = class Start {
  ...
  async process() {
    ...
    if (hasClient) {
      ...
      let args = ["run", "build", "--", "--watch", "--mode=development"];
      if (this.config.get("hot") === false) {
        args.push("--no-hot");
      }
      child_process.spawn("npm", args, {stdio: ["ignore", "ignore", "inherit"], shell: true, cwd: path.resolve(clientDir)});
    }
    ...
    if (TestMongo) {
      ...
      for (let dataStore in dataStoreMap) {
        ...
        if (!connectionUri.startsWith("mongo") || connectionUri.startsWith("mongodb+srv:")) {
          continue;
        }
