"use strict";

const { Config, Mutex } = require("uu_appg01_core-utils");
const { MongoMemoryServer, MongoMemoryReplSet } = require("mongodb-memory-server-core");
const { MongoClient } = require("mongodb");
const Path = require("path");
const Os = require("os");

const DEFAULT_DB = "test";
const DEFAULT_VERSION = "5.0.26";
const MONGO_BIN_PATH = Path.resolve(Os.homedir(), ".cache", "mongodb-binaries");

const MONGO_CLIENT_OPTS = {
};

class TestMongo {

  constructor(options = {}) {
    this._dbName = options.db || DEFAULT_DB;
    this._clients = {};
    this._connectionString = null;
    let config = {
      dbName: this._dbName,
    };
    if (options.port) {
      config.port = options.port;
    }
    if (options.dbPath) {
      config.dbPath = options.dbPath;
    }
    let storageEngine = options.storageEngine || Config.getString("embeddedMongoStorageEngine");
    if (storageEngine) {
      config.storageEngine = storageEngine;
    }
    let version = options.version || Config.getString("embeddedMongoDbVersion") || DEFAULT_VERSION;

    let replSet = options.replSet || Config.get("embeddedMongoReplSet");
    if (replSet) {
      if (typeof replSet === "boolean" || typeof replSet === "number") {
        this._replSet = {
          dbName: `${this._dbName}ReplSet`,
          name: `${this._dbName}ReplSet`
        };
      } else {
        this._replSet = replSet;
      }

      delete config.dbName;
      let opts = {
        instanceOpts: [config],
        binary: {
          version: version,
          downloadDir: MONGO_BIN_PATH,
        },
        replSet: this._replSet
      };
      this._mongodPromise = MongoMemoryReplSet.create(opts);
    } else {
      let opts = {
        instance: config,
        binary: {
          version: version,
          downloadDir: MONGO_BIN_PATH,
        },
      }; 
      this._mongodPromise = MongoMemoryServer.create(opts);
    }
  }

  isRunning() {
    return !!(this._mongod && (this._mongod.instanceInfo || this._mongod.instanceOpts));
  }

  getPort() {
    if (!this.isRunning()) {
      throw new Error("Test database is not running.");
    }
    return this._mongod.instanceInfo.port;
  }

  getDbPath() {
    if (!this.isRunning()) {
      throw new Error("Test database is not running.");
    }
    return this._mongod.instanceInfo.dbPath;
  }

  getConnectionString(db = null) {
    if (!this.isRunning()) {
      throw new Error("Test database is not running.");
    } else if (db) {
      return this._connectionString.replace(/[^/]*$/, db);
    } else {
      return this._connectionString;
    }
  }

  async start() {
    if (!this._mongod && this._mongodPromise) {
      this._mongod = await this._mongodPromise;
    }
    this._connectionString = this._mongod.getUri().replace(/[^/]*$/, this._dbName);
    if (this._replSet) {
      let client = await this.getClient("admin", {});
      let setName = (await client.db().command({ isMaster: 1 })).setName;
      if (!setName) {
        await client.db().command({
          replSetInitiate: {
            _id: this._replSet,
            members: [
              {
                _id: 0,
                host: `127.0.0.1:${this.getPort()}`,
              },
            ],
          },
        });
      }
      let waited = 0;
      while (!(await client.db().command({ isMaster: 1 })).ismaster) {
        if (waited >= 10000) {
          throw new Error("Replica set was not initiated in 10s.");
        }
        waited += await Mutex.sleep(100);
      }
    }
    return this._connectionString;
  }

  async stop() {
    let results = [];
    let clients = this._clients;
    Object.keys(this._clients).map(function (db) {
      let client = clients[db];
      results.push(client.close());
    });
    await Promise.all(results);
    await this._mongod.stop();
    this._clients = {};
    this._connectionString = null;
  }

  async clear(db = null) {
    let client = await this.getClient(db);
    await client.db().dropDatabase();
  }

  async getClient(db = null, mongoClientOpts = null) {
    if (typeof db !== "string" && mongoClientOpts == null) {
      mongoClientOpts = db;
      db = null;
    }
    db ||= this._dbName;
    let connectionString = this.getConnectionString(db);
    let client = this._clients[db];
    if (!client) {
      client = await MongoClient.connect(connectionString, { ...MONGO_CLIENT_OPTS, ...mongoClientOpts });
      this._clients[db] = client;
    }
    return client;
  }

}

module.exports = TestMongo;
