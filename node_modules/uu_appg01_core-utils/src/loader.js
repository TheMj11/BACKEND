...
class Loader {
  ...
  static loadRealization(sourcePath, realization, getMethod = true) {
    ...
    const splitString = (index, str, includeIndex = false) => [str.substring(0, index + (includeIndex ? 1 : 0)), str.substring(index + 1)];
    realization = realization.replace(/\\/g, "/");
    let clazzPath, clazzAndMethod;
    let separatorIdx = realization.lastIndexOf("/");
    if (separatorIdx >= 0) {
      ([clazzPath, clazzAndMethod] = splitString(separatorIdx, realization));
    } else {
      separatorIdx = realization.lastIndexOf(":");
      if (separatorIdx >= 0) {
        ([clazzPath, clazzAndMethod] = splitString(separatorIdx, realization, true));
      } else {
        ([clazzPath, clazzAndMethod] = ["", realization]);
      }
    }
