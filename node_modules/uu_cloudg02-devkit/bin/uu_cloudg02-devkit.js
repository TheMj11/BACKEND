...
const Fs = require("fs");
const DevkitBaseError = require("../src/errors/devkit-base-error.js");
...
let envRegex = /(?:-e|--env(?:ironment)?)[\s=]([^\s]*)/;
let envMatch = argsLine.match(envRegex);
if (envMatch) {
  // If environment is declared, remove it from arguments
  environment = envMatch[1];
  args = argsLine.replace(envRegex, "").split(/\s+/);
} else {
...
Process.on("uncaughtException", e => {
  if (e instanceof DevkitBaseError) {
