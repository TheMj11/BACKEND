...
function _saveLogs(logs, argv) {
  ...
  if (!argv.isInProjectScope) {var logEnv = (argv.appDeploymentUri ? argv.appDeploymentUri : argv.asid)
    .toLowerCase()
    .replace(/\[[a-zA-Z0-9]*\]/g, "")
    .replace(/\-|\_|\:/g, ".");
  }else {var logEnv = argv.environment;}
...
function _formatLog(log) {
  ...
  if (log.correlatedRecords != null && log.correlatedRecords.length > 0) {
    ...
    message = log.correlatedRecords.reduce((msg, rec) => {
