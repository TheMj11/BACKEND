...
const SysQosErrors = require("../api/errors/sys-qos-bad-request-error.js");
...
class QosMiddleware {
  ...
  async pre(req, res, next) {
    let rawUri = res.locals._uu_app_uve_handler_orig_url || res.locals.uri;
    let parsedUri;
    try {
      parsedUri = Uri.parse(rawUri);
    } catch (err) {
      return next(new SysQosErrors.SysQosBadRequestError.BadRequest(null, null, { uri: rawUri }, null, err));
    }
    ...
    if (Object.entries(uuAppErrorMap).length > 0) {
      return this._redirectWithRateIndication(req, res, next, parsedUri, uuAppErrorMap);
    }
    return next();
  }
