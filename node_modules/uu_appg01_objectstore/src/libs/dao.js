...
const INSERT_OID_ENABLED = Config.getBoolean("uu_app_objectstore_insert_oid");
...
class Dao extends UuDataStoreDao {
  ...
  async insertOne(uuObject, lock = null) {
    ...
    if (INSERT_OID_ENABLED){
      uuObject = this._preInsert(uuObject, this.collectionName);
    }
    ...
    if (check !== true) {
      ...
      return await db
        ...
        .insertOne(uuObject)
        .then((res) => {
          super._recalculateCurrentNoiAfter(1, uuObject["awid"]);
          uuObject["_id"] = res.insertedId;
          let insertedObject = super._convertToId(uuObject);
          if (lock) {
  ...
  /**
   *  Add additional attributes to uuObject which will be required after upgrade to uuAppObjectStoreg02.
   *  Can be overriden in child classes to change the behavior.
   * @param uuObject {object} - object to be inserted
   * @param objectType {string} specifying the object type in collection (joke, category, etc.), if not provided, collectionName is used
   * @protected
   */
  _preInsert(uuObject, objectType) {
    const _id = uuObject["_id"];
    const oid = uuObject["oid"];

    if (!oid && !_id) {
      uuObject["_id"] = new ObjectId();
      uuObject["oid"] = uuObject["_id"];
    } else if (oid && !_id) {
      uuObject["_id"] = oid instanceof ObjectId ? oid : new ObjectId(oid);
    } else if (!oid && _id) {
      uuObject["oid"] = _id;
    }
    // uuObject["sys"]["lockHash"] = null;
    // uuObject["sys"]["lockExp"] = null;
    uuObject["sys"]["uuObjectType"] = this._getObjectType(uuObject, objectType);

    return uuObject;
  }

  /**
   * Get object type for uuObject. Will be required in uuAppObjectStoreg02. Default is collectionName.
   * Can be overriden in child classes to change the object type.
   * @param uuObject {object} - object provide information about inserted object
   * @param objectType {string} - object type in collection
   * @returns {string} object type
   * @protected
   */
  _getObjectType(uuObject, objectType) {
    return objectType;
  }
  ...
  async insertMany(uuObjectList, ordered = true) {
    ...
    if (check !== true) {
      ...
      let nInserted = result.insertedCount;
      if (nInserted === 0) {
        //nothing inserted
        uuObjectList = [];
      } else {
        insertedIds = result.insertedIds ? Object.keys(result.insertedIds).map(function (key) { return result.insertedIds[key] }) : [];
        if (ordered) {
          //ordered inserts
          insertedIds = insertedIds.slice(0, nInserted);
          uuObjectList = uuObjectList.slice(0, nInserted);
          if (result.getWriteErrors()[0]) {
            error = result.getWriteErrors()[0]["errmsg"];
          }
        } else {
          //unordered inserts
          let indexes = result.getWriteErrors().map(function(error) {
            return error["index"];
          });
          ...
          let error = result.getWriteErrors().map(function(error) {
            return error["errmsg"];
          });
        }
      }
    ...
    return await db
      ...
      .then((result) => {
        return super._findWrapper(filter).then((res) => {
          if (res.length > result.deletedCount) {
            throw new ObjectLocked();
          } else {
            return result;
          }
  ...
  async findOneAndUpdate(filter, uuObject, revisionStrategy = "REVISION", lock = null) {
    ...
    return await db
      .collection(this.collectionName)
      .findOneAndUpdate(updateFilter, update, { returnDocument: "after", includeResultMetadata: true })
      .then((result) => {
        if (result.value) {
