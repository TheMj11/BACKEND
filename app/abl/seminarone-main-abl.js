...
const Errors = require("../api/errors/seminarone-main-error.js");
...
const logger = LoggerFactory.get("SeminaroneMainAbl");

class SeminaroneMainAbl {
  constructor() {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = [Schemas.SEMINARONE_MAIN, Schemas.STUDENT];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "UunSeminarone",
        typeCode: "uun-seminarone-maing01",
        location: location,
...
module.exports = new SeminaroneMainAbl();
